...
export function MobileMenu({ isOpen, onClose }: MobileMenuProps) {
  ...
  const sections = [
    ...
    { name: "Solutions", data: solutionsDetails },
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 z-[80] bg-black/70 transition-opacity duration-300 lg:hidden ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
          }`}
        onClick={handleClose}
      />
      ...
      {/* Drawer */}
      <div
        className={`fixed right-0 top-0 z-[90] flex h-full w-full max-w-sm transform flex-col overflow-y-auto border-l border-gray-200 bg-white shadow-2xl transition-transform duration-300 ease-in-out lg:hidden ${isOpen ? "translate-x-0" : "translate-x-full"
          }`}
      >
              ...
              <div key={section.name} className="border-b border-gray-100 pb-4">
                ...
                >
                  ...
                  <svg
                    ...
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    className={`transition-transform duration-300 ${openSection === section.name ? "rotate-180 text-red-600" : "text-gray-400"
                      }`}
                  >
                ...
                {/* Accordion Content */}
                <div
                  className={`grid transition-all duration-300 ease-in-out ${openSection === section.name ? "grid-rows-[1fr] opacity-100 mt-4" : "grid-rows-[0fr] opacity-0"
                    }`}
                >
