...
import { ArrowRight, Radar, ShieldCheck, type LucideIcon } from "lucide-react";
...
const solutionCards: SolutionCard[] = [
  {
    ...
    description:
      "Centralize threat intelligence, analyst workflows, and operational visibility in one secure platform.",
    href: "/solutions/keystone-arena",
    icon: Radar,
  },
  {
    title: "SOC Subscription",
    subtitle: "Coming soon",
    description:
      "A managed subscription model for teams that want continuous SOC support without rebuilding their stack.",
    href: "/solutions/soc-subscription",
    icon: ShieldCheck,
  },
];
    ...
    <main className="min-h-screen bg-white text-zinc-950">
      ...
      >
        ...
        <motion.div variants={stagger} className="relative z-10 mx-auto max-w-7xl">
          ...
          <motion.h1 className="max-w-4xl text-3xl font-black tracking-tight sm:text-4xl md:text-6xl">
            Solutions built for real operational teams.
          </motion.h1>
          <motion.p className="mt-6 max-w-3xl text-base leading-7 text-zinc-300 sm:text-lg md:text-xl md:leading-8">
            Explore Keystone’s solution stack, from CTI coordination to a managed SOC subscription path.
          </motion.p>
        </motion.div>
      ...
      >
        <div className="mx-auto max-w-7xl">
          <SectionDivider title="SOLUTIONS" className="mb-12" />
          <motion.div variants={stagger} className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {solutionCards.map((card) => {
              const Icon = card.icon;

              return (
                <motion.div key={card.href} whileHover={{ y: -8, scale: 1.01 }} className="scroll-mt-28">
                  <Link
                    href={card.href}
                  ...
                  >
                    ...
                    <h2 className="text-xl font-bold mb-4 text-gray-900 leading-tight mt-3">{card.title}</h2>
                    <p className="text-gray-600 text-sm md:text-base leading-relaxed mb-8 flex-grow">{card.description}</p>
        ...
        <div className="relative z-10 mx-auto flex max-w-7xl flex-col gap-8 md:flex-row md:items-center md:justify-between">
          <motion.div variants={stagger} className="max-w-3xl">
            ...
            <motion.h2 className="text-3xl font-black tracking-tight md:text-5xl">
              Do not choose a solution in the dark.
            </motion.h2>
            <motion.p className="mt-5 text-base leading-8 text-zinc-300 md:text-lg">
              Work with Keystone to map the right solution path for CTI operations or managed SOC support.
            </motion.p>
          </motion.div>
