...
import { ArrowRight, Clock3, Mail, ShieldAlert, type LucideIcon } from "lucide-react";
...
const updates: SubscriptionCard[] = [
  ...
  },
  {
    ...
    description: "Service scope and delivery packaging are being finalized so the offer can launch cleanly.",
    icon: ShieldAlert,
  },
  {
    title: "Priority updates",
    description: "We will route interested teams through the contact team and share launch details directly.",
    icon: Mail,
  },
];

export default function SocSubscriptionPage() {
  return (
    <main className="min-h-screen bg-white text-zinc-950">
      ...
      >
        <div className="mx-auto max-w-7xl">
          <SectionDivider title="SUBSCRIPTION STATUS" className="mb-12" />
          <motion.div variants={stagger} className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
            {updates.map((update) => {
              const Icon = update.icon;

              return (
                <motion.div key={update.title} whileHover={{ y: -8, scale: 1.01 }} className="scroll-mt-28">
                  <div className="bg-white rounded-[2rem] p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgba(220,38,38,0.1)] hover:-translate-y-2 transition-all duration-300 group border border-gray-100 flex flex-col h-full block">
                    ...
                    <h2 className="text-xl font-bold mb-4 text-gray-900 leading-tight">{update.title}</h2>
                    <p className="text-gray-600 text-sm md:text-base leading-relaxed mb-8 flex-grow">{update.description}</p>
