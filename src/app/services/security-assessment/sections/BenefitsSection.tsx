...
import { LockIcon as Shield } from "@/components/animate-ui/icons/lock";
...
export function BenefitsSection() {
  ...
  const benefits = [
    {
      title: "Expose Real Weaknesses",
      desc: "Find exploitable misconfigurations, excessive access, exposed services, and brittle architecture before they become incidents.",
      icon: <Eye className="w-5 h-5" animateOnHover={true} />,
      color: "from-red-500 to-red-600",
    },
    {
      title: "Validate Critical Assets",
      desc: "Assess the infrastructure, cloud platforms, identity systems, and operational environments that matter most to the business.",
      icon: <Shield className="w-5 h-5" animateOnHover={true} />,
      color: "from-orange-500 to-orange-600",
    },
    {
      title: "Prioritize Risk",
      desc: "Separate noise from urgent exposure with evidence-led severity, business impact, and remediation sequencing.",
      icon: <Target className="w-5 h-5" animateOnHover={true} />,
      color: "from-red-600 to-orange-600",
    },
    {
      title: "Accelerate Hardening",
      desc: "Translate findings into technical guidance your infrastructure, cloud, and security teams can execute quickly.",
      icon: <Zap className="w-5 h-5" animateOnHover={true} />,
      color: "from-red-500 to-orange-500",
    },
    {
      title: "Prove Readiness",
      desc: "Create a defensible view of security posture for leadership, auditors, regulators, and technical owners.",
          ...
          <div className="space-y-4 pt-4">
            {benefits.map((item, idx) => (
              ...
              >
                ...
                >
                  <div
                    className={`mt-0.5 p-2 rounded-md transition-colors shrink-0 ${
                      activeIdx === idx
                        ? "bg-red-50 text-red-600"
                        : "bg-zinc-100 text-zinc-400 group-hover:text-red-500 group-hover:bg-red-50/50"
                    }`}
                  >
                    {item.icon}
                  </div>
        ...
        >
          ...
          <div className="relative rounded-lg overflow-hidden shadow-2xl border border-zinc-800 bg-zinc-900 h-full w-full flex flex-col">
            ...
            <div className="h-10 bg-zinc-950 border-b border-zinc-800 flex items-center px-4 gap-2 z-20">
              ...
              <span className="font-medium text-[10px] text-zinc-600 ml-2">assessment_readiness_monitor.exe</span>
            </div>
            ...
            <div className="flex-1 relative bg-[radial-gradient(ellipse_at_center,rgba(220,38,38,0.1),transparent_70%)]">
              <AnimatePresence mode="wait">
                ...
                >
                  <div
                    className={`w-24 h-24 rounded-full bg-gradient-to-br ${benefits[activeIdx].color} mb-8 flex items-center justify-center p-0.5 shadow-[0_0_30px_rgba(220,38,38,0.3)]`}
                  >
                    <div className="w-full h-full bg-zinc-950 rounded-full flex items-center justify-center">
                      <div className="text-white scale-150">{benefits[activeIdx].icon}</div>
                    </div>
                  </div>

                  <motion.h2 className="text-2xl font-bold text-white mb-4 uppercase tracking-widest">
                    {benefits[activeIdx].title}
                  </motion.h2>

                  <div className="font-medium text-zinc-500 text-sm leading-loose max-w-sm">
                    {"// Running posture diagnostic..."}
                    <br />
